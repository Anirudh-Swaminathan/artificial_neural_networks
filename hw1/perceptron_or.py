...
"""Script to train a single-layer perceptron network to learn the OR function"""
...
class NeuralNetwork(object):
    ...
    def build_net(self, wts):
        ...
        for i in range(self.l1_num):
            n1 = Neuron(self.inp_list, wts[i], 0.5)
...
def train_net(nn):
    ...
    while cur_epoch < nb_epochs:
        ...
        fin_wt = nn.get_weight()
        if fin_wt == ini_wt:
            print("Weights no longer updating after epoch number ", cur_epoch + 1, "!! Training can stop!!")
            break
        cur_epoch += 1
    if cur_epoch == 10:
        print("10 epochs completed. Training of the OR function is complete!!")


def main():
    """The main method"""
    print("Building an OR gate using Perceptron Neural Network that is trained")
